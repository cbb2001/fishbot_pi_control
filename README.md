...
tip_peak = tip_center + b2 × delta_abs / 106 × 86
```

`b1=0` 时尖端全程精确保持中位。`b1=1` 时尖端在 `0..t/2` 从中位平滑到
`tip_peak`，在 `t/2..t` 平滑回中位。左侧尖端中位为 `94°`，右侧为 `90°`；
右侧不会因镜像结构擅自反转 `b2`。

20260725 独立配置段只保存与现有字段不等价的 `theta` 范围和 `106/86` 映射标尺。
七路 `85/95/95/121/94/143/90` 参考中位直接复用
